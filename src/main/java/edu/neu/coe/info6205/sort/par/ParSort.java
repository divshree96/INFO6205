...
class ParSort {
    ...
    public static void sort(int[] array, int from, int to,ForkJoinPool forkJoinPool) {
        if (to - from < cutoff) Arrays.sort(array, from, to);
            ...
            CompletableFuture<int[]> parsort1 = parsort(array, from, from + (to - from) / 2,forkJoinPool); // TO IMPLEMENT
            CompletableFuture<int[]> parsort2 = parsort(array, from + (to - from) / 2, to,forkJoinPool); // TO IMPLEMENT
    ...
    private static CompletableFuture<int[]> parsort(int[] array, int from, int to,ForkJoinPool forkJoinPool) {
        return CompletableFuture.supplyAsync(
                () -> {
                    ...
                    sort(result, 0, to - from,forkJoinPool);
                    return result;
                },forkJoinPool
        );
    }
